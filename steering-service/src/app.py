...
class Main:
    ...
    def _register_routes(self):
        ...
        def do_remote_steering(name: str):
            ...
            resp = dash_parser.build(
                target=target, nodes=nodes_p, uri=uri, request=request
            )
    ...
    def run(self):
        ...
        cert, key = (
            os.path.join(certs_dir, "steering-service.pem"),
            os.path.join(certs_dir, "steering-service-key.pem"),
        )
        try:
...
if __name__ == "__main__":
    ...
    main_app = Main(
        selector_instance, current_strategy_name, active_log_filename, args.log_suffix
    )
