...
def format_plot_aggregated(ax, title, xlabel, ylabel, legend_loc='best',
    ...
    if custom_legend_handles and custom_legend_labels:
        ax.legend(custom_legend_handles, custom_legend_labels, loc=legend_loc, fontsize=10, framealpha=0.9, edgecolor='gray', fancybox=True)
    elif ax.has_data() and ax.get_legend_handles_labels()[0]:
        ax.legend(loc=legend_loc, fontsize=10, framealpha=0.9, edgecolor='gray', fancybox=True)
    ax.grid(True, linestyle=':', alpha=0.6, which='major')
    if y_log_scale and ax.has_data(): ax.grid(True, linestyle=':', alpha=0.3, which='minor')
    plt.tight_layout()

def generate_plots_for_aggregated(csv_file_path: str, max_time: float = None):
    ...
    if max_time is None:
        xlim_for_plots = df_agg['sim_time_client'].max() if 'sim_time_client' in df_agg.columns else 150
    else:
        xlim_for_plots = max_time
    ...
    logger.info(f"Generating plots for time range: 0-{xlim_for_plots}s")
    ...
    if 'experienced_latency_ms' in df_agg.columns and 'dynamic_best_server_latency' in df_agg.columns:
        ...
        format_plot_aggregated(ax1, f"Average Chosen Server Latency vs Optimal Latency\nStrategy: {strategy_display_name}",
                               "Average Simulation Time (s)", "Average Latency (ms)", legend_loc='best',
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_time", type=float, default=None, help="Maximum simulation time to plot (seconds). If not specified, uses all available data.")
    ...
    if os.path.exists(csv_path):
        ...
        generate_plots_for_aggregated(csv_path, max_time=args.max_time)
    else:
